...
exports.uploadToS3 = async (file, folderName = 'movies') => {
...
exports.processImageUploads = async (req, res, next) => {
  try {
    ...
    if (req.files.poster && req.files.poster.length > 0) {
      const posterUrl = await exports.uploadToS3(req.files.poster[0], 'posters');
      imageUrls.poster_path = posterUrl;
    }
    
    // Process backdrop image if it exists
    if (req.files.backdrop && req.files.backdrop.length > 0) {
      const backdropUrl = await exports.uploadToS3(req.files.backdrop[0], 'backdrops');
